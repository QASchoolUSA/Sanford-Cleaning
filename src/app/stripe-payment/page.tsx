...
export default function StripePaymentPage() {
  ...
  const serviceParam = searchParams.get("service") || "Cleaning Service";
  ...
  const startCheckout = async () => {
    try {
      ...
      const resp = await fetch("/api/stripe/create-checkout-session", {
        ...
        body: JSON.stringify({ amount, service: serviceParam, customerEmail }),
      });
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-xl mx-auto bg-white rounded-2xl shadow-lg border border-gray-100 p-8">
            ...
            <div className="bg-gray-50 rounded-lg p-4 mb-6">
              <p className="text-gray-700"><span className="font-semibold">Service:</span> {serviceParam}</p>
              <p className="text-gray-700"><span className="font-semibold">Amount:</span> {amount && !isNaN(amount) ? `$${amount.toFixed(2)}` : "Not provided"}</p>
              {customerEmail && (
                <p className="text-gray-700"><span className="font-semibold">Email:</span> {customerEmail}</p>
              )}
            </div>
